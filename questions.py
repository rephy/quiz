...
class Questions:
    ...
    def new_questions(self):
        self.questions = requests.get("https://opentdb.com/api.php", params={"amount": self.num_of_questions, "difficulty": self.difficulty_level, "encode": "base64"}).json()['results']
        for question in self.questions:
            question['type'] = base64.b64decode(question['type']).decode("utf-8")
            question['difficulty'] = base64.b64decode(question['difficulty']).decode("utf-8")
            question['category'] = base64.b64decode(question['category']).decode("utf-8")
            ...
            for n in range(0, len(question['incorrect_answers'])):
                question['incorrect_answers'][n] = base64.b64decode(question['incorrect_answers'][n]).decode("utf-8")
        new_line()
